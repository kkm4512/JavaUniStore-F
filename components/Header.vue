  ...
  <nav class="bg-white border-gray-200 dark:bg-gray-900">
    <div class="max-w-screen-xl flex items-center justify-between mx-auto p-4">
      <NuxtLink to="/" class="flex items-center space-x-3 rtl:space-x-reverse">
        <img :src="logoPath" class="h-[4rem]" alt="Flowbite Logo" />
      </NuxtLink>
      <button
      ...
      <div class="hidden w-full md:block md:w-auto" id="navbar-default">
        ...
        <div v-if="isLoginButtonClicked">
          <LoginModal :isLoginButtonClicked="isLoginButtonClicked" />
        </div>
      </div>
    </div>
  </nav>
...
import { DEFAULT_LOGO_PATH } from "~/path/paths";
const logoPath = DEFAULT_LOGO_PATH;

/**
 * 1. 로그인 선택여부
 * 2. true면 LoginModal창 띄우기
 */

const isLoginButtonClicked = ref<boolean>(false);
function loginToggle(){
  isLoginButtonClicked.value = !isLoginButtonClicked.value;
}
